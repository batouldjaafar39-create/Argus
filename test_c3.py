...
ALERT = {
    "alert_id": "A1",
    "summary": "Unusual Kerberos activity observed",
    "source": "IDS",
}
...
def _script(responses):
    ...
    The real _run_llama_cli now accepts an optional max_tokens argument,
    so the mock must accept it too.
    """
    calls = {"n": 0}

    def fake(prompt, config, max_tokens=None):
        i = calls["n"]
        calls["n"] += 1

        if i >= len(responses):
            raise AssertionError(
                "llama-cli called more times than scripted"
            )
...
def test_immediate_final_report(monkeypatch, all_log_paths):
    monkeypatch.setattr(
        ...
        "_run_llama_cli",
        _script([
            _final_report_json("done immediately"),
        ]),
    )
...
def test_one_tool_call_then_final_report(monkeypatch, all_log_paths):
    ...
    monkeypatch.setattr(
        ...
        _script([
            tool_call,
            _final_report_json("used evidence"),
        ]),
    )
...
def test_multiple_tools_across_iterations(monkeypatch, all_log_paths):
    ...
    assert trace.tools_queried == [
        "query_conn",
        "query_kerberos",
    ]


def test_all_six_tools_reachable_in_one_investigation(
    monkeypatch,
    all_log_paths,
):
    tool_names = [
    ...
    calls.append(
        _final_report_json("toured all tools")
    )
...
def test_json_retry_succeeds_on_second_attempt(
    monkeypatch,
    all_log_paths,
):
    monkeypatch.setattr(
...
def test_json_retry_exhausted_terminates_gracefully(
    monkeypatch,
    all_log_paths,
):
    config = C3Config(
    ...
    assert "did not complete normally" in (
        trace.final_report["finding"]
    )
...
def test_json_extraction_recovers_extra_closing_brace():
    raw = (
        '{"action": "tool_call", "tool": "query_conn", '
        '"args": {}}}'
    )
...
def test_malformed_tool_call_args_triggers_recovery_not_crash(
    monkeypatch,
    all_log_paths,
):
    bad_call = json.dumps({
    ...
    assert trace.final_report["finding"] == (
        "recovered from bad args"
    )
...
def test_truncation_event_logged_in_trace(
    monkeypatch,
    all_log_paths,
):
    tool_call = json.dumps({
...
def test_conversation_prompt_is_bounded():
    ...
    bounded = c3_llm._bound_conversation(
        conversation,
        seed,
        100,
    )
...
def test_invalid_final_report_fed_back_and_corrected(
    monkeypatch,
    all_log_paths,
):
    ...
    invalid_events = [
        ...
        if event["event"] == "final_report_invalid"
    ]

    assert len(invalid_events) == 1


def test_final_report_missing_required_field_rejected(
    monkeypatch,
    all_log_paths,
):
    bad_report = json.dumps({
...
def test_persistently_invalid_final_report_hits_iteration_limit(
    monkeypatch,
    all_log_paths,
):
    bad_report = json.dumps({
...
def test_invalid_action_value_recovered(
    monkeypatch,
    all_log_paths,
):
...
# ---------------------------------------------------------------------
# Tool dispatch failures inside the loop
# ---------------------------------------------------------------------

def test_tool_call_failure_fed_back_not_fatal(
    monkeypatch,
    all_log_paths,
):
    bad_call = json.dumps({
...
def test_trace_save_writes_valid_json(
    tmp_path,
    monkeypatch,
    all_log_paths,
):
    ...
    assert (
        isinstance(loaded["events"], list)
        and len(loaded["events"]) > 0
    )


def test_llm_call_failure_distinct_from_json_parse_failure(
    monkeypatch,
    all_log_paths,
):
    """
    A backend failure must remain distinct from a genuine JSON
    parsing failure.
    """
    ...
    def raise_backend_error(
        ...
        raise c3_llm.LlamaCliError(
            "llama-cli timed out after 300s"
        )
    ...
    assert trace.terminated_reason == (
        "llm_call_failure"
    )

    assert "backend call itself failed" in (
        trace.final_report["limitations"]
    )

    assert trace.json_retry_count == 0


def test_subprocess_backend_passes_stop_sequence(
    monkeypatch,
):
    """
    Regression test for the subprocess stop sequence.
    """
...
def test_trace_summary_counts_match_events(
    monkeypatch,
    all_log_paths,
):
    tool_call = json.dumps({
